...
export function App(){
 ...
 <Panel title={tab==='mapping'?'实时地图':'巡航地图'} className="map-panel" extra={<div className="map-head-tools">{tab==='cruise'&&mapView==='map'&&mapTools}{tab==='cruise'&&mapView==='map'&&<span className={'localization-state '+(s?.localization.ready?'online':'')}>{s?.mode==='navigation'&&!s?.navigation?.ready?'导航初始化':s?.localization.ready?'已定位':s?.mode==='navigation'?'待定位':'未加载'}</span>}<div className="segmented compact"><button className={mapView==='map'?'active':''} onClick={()=>setMapView('map')}>地图</button><button className={mapView==='rviz'?'active':''} onClick={()=>{setMapView('rviz');setTool('browse')}}>RViz</button></div><button className={'icon-button'+((avoidance?.enabled??true)?' on':'')} aria-label={(avoidance?.enabled??true)?'关闭避障（贴线巡航）':'开启避障'} title={(avoidance?.enabled??true)?'避障已开启：离障碍保持余量、绕开障碍。点击关闭后按教学路线贴线行驶（有碰撞风险）':'避障已关闭：按路线贴线行驶，不绕障、不因贴近墙而拒绝起步；有碰撞风险'} disabled={!actionAllowed||s?.mode!=='navigation'} onClick={toggleAvoidance}>{(avoidance?.enabled??true)?<Shield size={15}/>:<ShieldOff size={15}/>}</button><button className={'icon-button'+(aruco?.seeing?' on':'')} aria-label="记录标签位置" title={aruco?(aruco.configured?`标签 #${aruco.marker_id} 已标定${aruco.seeing?'（当前可见）':'（当前看不到）'}；再点一次可重新标定`:'把车对准标签后点这里标定标签地图位置'):'相机标签定位未启用'} disabled={!actionAllowed||!mapLoaded||!aruco?.seeing||(s?.localization.match??0)<.8} onClick={captureMarker}><QrCode size={15}/></button><button className={'icon-button'+(refine?.enabled?' on':'')} aria-label={refine?.enabled?'关闭静止校准':'开启静止校准'} title={(refine?.enabled?'关闭':'开启')+'静止校准：定位准后底盘静止时周期性吸附校正'} disabled={!actionAllowed||s?.mode!=='navigation'} onClick={toggleRefine}><Magnet size={15}/></button><button className="icon-button" aria-label={expanded?'退出全屏地图':'全屏地图'} title={expanded?'退出全屏地图':'全屏地图'} onClick={()=>setExpanded(v=>!v)}>{expanded?<Minimize2 size={15}/>:<Maximize2 size={15}/>}</button></div>}>
