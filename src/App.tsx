...
import {Map,Route as RouteIcon,Settings,Square,Save,RotateCcw,RotateCw,Magnet,CircleDot,Play,Pause,LocateFixed,Trash2,Camera,Radio,Wifi,WifiOff,X,Check,LoaderCircle,Plus,ArrowUp,ArrowDown,ArrowLeft,ArrowRight,Activity,Maximize2,Minimize2,Crosshair} from 'lucide-react';
...
export function App(){
 ...
 const applyRecorded=()=>{if(!recordedPoints.length)return;setPoints(recordedPoints);setRouteId('');setToast({message:`已用记录的 ${recordedPoints.length} 个点作为航点，可直接保存为路线`,error:false})};
 ...
 <Panel title={tab==='mapping'?'实时地图':'巡航地图'} className="map-panel" extra={<div className="map-head-tools">{tab==='cruise'&&mapView==='map'&&mapTools}{tab==='cruise'&&mapView==='map'&&<span className={'localization-state '+(s?.localization.ready?'online':'')}>{s?.mode==='navigation'&&!s?.navigation?.ready?'导航初始化':s?.localization.ready?'已定位':s?.mode==='navigation'?'待定位':'未加载'}</span>}<div className="segmented compact"><button className={mapView==='map'?'active':''} onClick={()=>setMapView('map')}>地图</button><button className={mapView==='rviz'?'active':''} onClick={()=>{setMapView('rviz');setTool('browse')}}>RViz</button></div><button className={'icon-button'+(refine?.enabled?' on':'')} aria-label={refine?.enabled?'关闭静止校准':'开启静止校准'} title={(refine?.enabled?'关闭':'开启')+'静止校准：定位准后底盘静止时周期性吸附校正'} disabled={!actionAllowed||s?.mode!=='navigation'} onClick={toggleRefine}><Magnet size={15}/></button><button className="icon-button" aria-label={expanded?'退出全屏地图':'全屏地图'} title={expanded?'退出全屏地图':'全屏地图'} onClick={()=>setExpanded(v=>!v)}>{expanded?<Minimize2 size={15}/>:<Maximize2 size={15}/>}</button></div>}>
 ...
 <div className="waypoint-tools"><button className={'record-button'+(record?.active?' active':'')} aria-label={record?.active?'停止记录航迹':'开始记录航迹'} title={(record?.active?'停止记录':'记录航迹')+'：遥控行驶时每 20 cm 记一个点（重新开始会清空上次记录）'} disabled={!actionAllowed||!s?.pose} onClick={toggleRecord}><CircleDot size={14}/>{record?.active?`停止记录 ${record.count} 点 / ${(record.distance_m||0).toFixed(1)} m`:'记录航迹'}</button><button className="record-button" aria-label="用记录的点作为航点" title={`把记录的 ${record?.count||0} 个点填入航点列表，再点保存路线`} disabled={!recordedPoints.length||locked} onClick={applyRecorded}><RouteIcon size={14}/>用为航点</button></div>
