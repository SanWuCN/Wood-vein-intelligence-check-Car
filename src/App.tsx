...
import {Map,Route as RouteIcon,Settings,Square,Save,RotateCcw,RotateCw,Magnet,CircleDot,Sparkles,QrCode,ShieldOff,Shield,Play,Pause,LocateFixed,Trash2,Camera,Wifi,WifiOff,X,Check,LoaderCircle,Plus,ArrowUp,ArrowDown,ArrowLeft,ArrowRight,Activity,Maximize2,Minimize2,Crosshair} from 'lucide-react';
...
export function App(){
 ...
 const {state:s,connected,canControl}=useRobot();const [tab,setTab]=useState<'mapping'|'cruise'>(initialTab),[tool,setTool]=useState<'browse'|'pose'|'waypoint'>('browse'),[expanded,setExpanded]=useState(false);
 const [maps,setMaps]=useState<SavedMap[]>([]),[routes,setRoutes]=useState<Route[]>([]),[selectedMap,setSelectedMap]=useState(''),[points,setPoints]=useState<Waypoint[]>([]),[mode,setMode]=useState<MissionMode>('multi'),[speed,setSpeed]=useState(.05);
 ...
 const maxSpeed=Math.min(.2,s?.max_speed_mps??.2);
 ...
 useEffect(()=>{if(s?.speed_mps!=null)setSpeed(Math.max(0,Math.min(maxSpeed,s.speed_mps)))},[s?.speed_mps,maxSpeed]);
 ...
 const changeTab=(t:'mapping'|'cruise')=>{setTab(t);setTool('browse');refresh().catch(()=>{})};
 ...
 const loadMap=(saveCurrent?:boolean)=>run('load',async()=>{
   ...
   setPoints([]);setRouteId('');setTool('browse');setModal(null);await refresh();
 ...
 const avoidance=s?.avoidance;
 const toggleAvoidance=()=>run('avoidance',async()=>{const next=!(avoidance?.enabled??true);
   const r=await request<{avoidance:{enabled:boolean;ok?:boolean}}>('/navigation/avoidance',{enabled:next});
   setToast({message:r.avoidance.enabled?'已开启避障（离障碍保持余量）':'已关闭避障：贴线巡航，不再绕障——请自行注意碰撞风险'+(r.avoidance.ok===false?'（代价地图未就绪，将在下次加载地图时生效）':''),error:false})},'');
 ...
 <Panel title={tab==='mapping'?'实时地图':'巡航地图'} className="map-panel" extra={<div className="map-head-tools">{tab==='cruise'&&mapTools}{tab==='cruise'&&<span className={'localization-state '+(s?.localization.ready?'online':'')}>{s?.mode==='navigation'&&!s?.navigation?.ready?'导航初始化':s?.localization.ready?'已定位':s?.mode==='navigation'?'待定位':'未加载'}</span>}<button className={'icon-button'+((avoidance?.enabled??true)?' on':'')} aria-label={(avoidance?.enabled??true)?'关闭避障（贴线巡航）':'开启避障'} title={(avoidance?.enabled??true)?'避障已开启：离障碍保持余量、绕开障碍。点击关闭后按教学路线贴线行驶（有碰撞风险）':'避障已关闭：按路线贴线行驶，不绕障、不因贴近墙而拒绝起步；有碰撞风险'} disabled={!actionAllowed||s?.mode!=='navigation'} onClick={toggleAvoidance}>{(avoidance?.enabled??true)?<Shield size={15}/>:<ShieldOff size={15}/>}</button><button className={'icon-button'+(aruco?.seeing?' on':'')} aria-label="记录标签位置" title={aruco?(aruco.configured?`标签 #${aruco.marker_id} 已标定${aruco.seeing?'（当前可见）':'（当前看不到）'}；再点一次可重新标定`:'把车对准标签后点这里标定标签地图位置'):'相机标签定位未启用'} disabled={!actionAllowed||!mapLoaded||!aruco?.seeing||(s?.localization.match??0)<.8} onClick={captureMarker}><QrCode size={15}/></button><button className={'icon-button'+(refine?.enabled?' on':'')} aria-label={refine?.enabled?'关闭静止校准':'开启静止校准'} title={(refine?.enabled?'关闭':'开启')+'静止校准：定位准后底盘静止时周期性吸附校正'} disabled={!actionAllowed||s?.mode!=='navigation'} onClick={toggleRefine}><Magnet size={15}/></button><button className="icon-button" aria-label={expanded?'退出全屏地图':'全屏地图'} title={expanded?'退出全屏地图':'全屏地图'} onClick={()=>setExpanded(v=>!v)}>{expanded?<Minimize2 size={15}/>:<Maximize2 size={15}/>}</button></div>}>
 <div className="map-view">
 <><MapCanvas meta={s?.map||null} pose={s?.pose||null} scan={s?.scan_points||[]} path={pathStale?[]:(s?.path||[])} points={tab==='cruise'?points:[]} record={tab==='cruise'?recordedPoints:[]} tool={tool} onPlace={place} enabled={actionAllowed&&mapLoaded&&!locked}/>{tab==='cruise'&&mapLoaded&&!locked&&<div className="map-nudge" role="group" aria-label="定位微调">{[[<RotateCcw size={13}/>,'逆时针 1.5°',0,0,-1,'逆时针微调'],[<ArrowUp size={13}/>,'向上 2 cm',0,1,0,'向上微调'],[<RotateCw size={13}/>,'顺时针 1.5°',0,0,1,'顺时针微调'],[<ArrowLeft size={13}/>,'向左 2 cm',-1,0,0,'向左微调'],[<ArrowDown size={13}/>,'向下 2 cm',0,-1,0,'向下微调'],[<ArrowRight size={13}/>,'向右 2 cm',1,0,0,'向右微调']].map(([icon,title,r,u,turn,label],i)=><button key={i} title={title as string} aria-label={label as string} disabled={!actionAllowed||busy==='nudge'} onClick={()=>nudge(r as number,u as number,turn as number)}>{icon}</button>)}</div>}</>
 {tab==='cruise'&&refineHint&&<span className="map-hint refine-hint"><Magnet size={12}/>{refineHint}</span>}
 {tab==='cruise'&&!!aruco?.count&&<span className="map-hint refine-hint" style={{right:8,top:68}}><QrCode size={12}/>相机标签校正 {aruco.count} 次{aruco.shift_m!=null?`（最近 ${aruco.shift_m.toFixed(2)} m / ${(aruco.shift_deg??0).toFixed(1)}°）`:''}</span>}
 {tab==='cruise'&&!!relocalize?.count&&(relocalize.count>0)&&<span className={'map-hint '+(relocalize.reason==='stopped'?'stale-hint':'refine-hint')} style={{right:8,top:38}}><Crosshair size={12}/>航行中自动校准 {relocalize.count} 次{relocalize.shift_m!=null?`（最近 ${relocalize.shift_m.toFixed(2)} m / ${(relocalize.shift_deg??0).toFixed(1)}°）`:''}{relocalize.reason==='stopped'?' · 已安全停车':''}</span>}
 {tab==='cruise'&&record?.active&&<span className="map-hint record-hint"><CircleDot size={12}/>记录中 {record.count} 点 / {(record.distance_m||0).toFixed(1)} m</span>}
 {tab==='cruise'&&!record?.active&&pathStale&&points.length>0&&(s?.path?.length||0)>0&&<span className="map-hint stale-hint">航点已改动，路径未更新，请重新“预览”</span>}
 ...
 <div className="speed-control"><label htmlFor="speed">巡航速度</label><input id="speed" type="range" min="0" max={maxSpeed} step="0.01" value={speed} disabled={!actionAllowed} onChange={e=>setSpeed(Number(e.target.value))} onPointerUp={e=>{const value=Number(e.currentTarget.value);run('speed',()=>request('/navigation/speed',{speed_mps:value}))}} onKeyUp={e=>{if(e.key.startsWith('Arrow')||['Home','End','PageUp','PageDown'].includes(e.key)){const value=Number(e.currentTarget.value);run('speed',()=>request('/navigation/speed',{speed_mps:value}))}}}/><strong>{speed.toFixed(2)}<small>m/s</small></strong></div>
 <div className="mission-actions"><button className="preview-button" disabled={!actionAllowed||!mapLoaded||!s?.localization.ready||!s?.navigation?.planner_ready||!points.length||locked} onClick={()=>run('preview',async()=>{const r=await request<{tight_turn_ratio?:number;min_radius?:number|null;nudged_cm?:number}>('/navigation/preview',{points,mode});setPathStale(false);setToast({message:(r.nudged_cm?`车离墙太近，已自动倒车约 ${r.nudged_cm} cm 让位；`:'')+(r.tight_turn_ratio&&r.tight_turn_ratio>0?`路径已生成；但路线有 ${Math.round(r.tight_turn_ratio*100)}% 的转弯比车的最小转弯半径(0.35m)更紧，规划可能绕行——建议点“整理航点”`:'路径已生成'),error:false})},'')}><RouteIcon size={14}/>{busy==='preview'?'规划中':'预览'}</button><button className="primary" disabled={!actionAllowed||!mapLoaded||!s?.localization.ready||!s?.navigation?.ready||!points.length||(mode==='loop'&&points.length<2)||running||speed===0} onClick={()=>run('mission',()=>s?.mission.state==='paused'?request('/navigation/resume',{speed_mps:speed}):request('/navigation/start',{map_id:selectedMap,points,mode,speed_mps:speed}))}><Play size={15} fill="currentColor"/>{s?.mission.state==='paused'?'继续':'开始巡航'}</button><button disabled={!actionAllowed||!running} onClick={()=>run('pause',()=>request('/navigation/pause',{}))}><Pause size={15}/>暂停</button></div>
 ...
 {modal==='waypoints'&&<WaypointEditor points={points} locked={locked||!actionAllowed} single={mode==='single'} canAdd={actionAllowed&&mapLoaded&&!locked} onClose={()=>setModal(null)} onApply={next=>{setPoints(next);setRouteId('');setModal(null)}} onAdd={next=>{setPoints(next);setRouteId('');setModal(null);setTool('waypoint')}}/>}
 ...
 {modal==='settings'&&<Modal title="连接设置" onClose={()=>setModal(null)}><form onSubmit={e=>{e.preventDefault();const b={platform_url:config.platform_url,camera_rtmp_url:config.camera_rtmp_url,...(tokenEdited?{platform_token:config.platform_token}:{})};run('settings-save',async()=>{await request('/settings/save',b);setModal(null)},'设置已保存')}}><label>设备编号<input value={config.device_id||''} readOnly/></label><label>平台地址<input type="url" placeholder="https://platform.example.com" value={config.platform_url||''} onChange={e=>setConfig(v=>({...v,platform_url:e.target.value}))}/></label><label>设备令牌<input type="password" autoComplete="new-password" placeholder={config.platform_token_set?'已设置':'未设置'} value={config.platform_token||''} onChange={e=>{setTokenEdited(true);setConfig(v=>({...v,platform_token:e.target.value}))}}/></label><label>摄像头 · RTMP<input placeholder="rtmp://host/live/camera" value={config.camera_rtmp_url||''} onChange={e=>setConfig(v=>({...v,camera_rtmp_url:e.target.value}))}/></label><div className="modal-actions"><button type="button" onClick={()=>setModal(null)}>取消</button><button className="primary" disabled={!!busy}><Check size={15}/>保存设置</button></div></form></Modal>}
 </div>
}
