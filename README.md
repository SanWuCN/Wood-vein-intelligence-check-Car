...
- **建图**：gmapping 启停、地图保存、重新建图自动备份、可交互实时栅格地图。
- **巡航**：地图加载、AMCL 自动/人工定位、点到点 / 多点 / 循环、路线保存、Nav2 路径预览、暂停 / 继续 / 停止、速度设置（0–0.2 m/s，默认 0.05 m/s；设为 0 暂停，调高后点击继续）。
- **静止自动校准**：定位就绪后底盘静止时，每 8 s 自动做一次 ±0.18 m / ±6° 小窗口吸附（任务中不干预，可一键关闭）。
...
- **视频**：摄像头 MJPEG 内嵌播放，可另配摄像头 RTMP 外推。
...
- 建图与巡航页均直接显示交互地图，可一键**全屏地图**（隐藏右栏，地图可视区 994×434）。
- 支持 `?tab=cruise`、`?tab=mapping` 直达；旧链接中的 `view` 参数忽略，统一显示地图。
